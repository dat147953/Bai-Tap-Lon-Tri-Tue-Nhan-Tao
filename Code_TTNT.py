    ...
    for table in seating:
        for i in range(len(table)):
            for j in range(i + 1, len(table)):
                score += relationship_matrix[guest_list.index(table[i])][guest_list.index(table[j])]
    return score
...
def generate_population(size):
    ...
    for _ in range(size):
        ...
        seating = [shuffled_guests[i:i + table_size] for i in range(0, num_guests, table_size)]
...
def order_crossover(parent1, parent2):
    ...
    remaining = [guest for guest in flat_parent2 if guest not in child]
    ...
    return [child[i:i + table_size] for i in range(0, num_guests, table_size)]

def mutate(seating):
    """Hoán đổi hai khách giữa hai bàn để giữ số lượng cân bằng."""
    table1, table2 = random.sample(range(len(seating)), 2)
    idx1, idx2 = random.randint(0, table_size - 1), random.randint(0, table_size - 1)
    seating[table1][idx1], seating[table2][idx2] = seating[table2][idx2], seating[table1][idx1]
    return seating
...
class SeatingApp:
    def __init__(self, root):
        ...
        self.label = tk.Label(root, text="Bảng điểm thân thiết giữa các khách mời", font=("Arial", 12, "bold"))
        ...
        for i in range(len(guest_list) + 1):
            for j in range(len(guest_list) + 1):
                ...
                tk.Label(self.table_frame, text=text, width=6, borderwidth=1, relief="solid").grid(row=i, column=j)
        
        self.button = tk.Button(root, text="Sắp xếp chỗ ngồi tối ưu", command=self.run_algorithm)
